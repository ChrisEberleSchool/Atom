...
glm::mat4 CameraComponent::GetViewMatrix() const {
  ...
  glm::vec3 targetPos =
      Target ? Target->transform.Position : position + GetForward();
...
glm::mat4 CameraComponent::GetProjectionMatrix(float aspectRatio) const {
  if (Type == CameraType::Perspective)
    ...
    return glm::ortho(-OrthoWidth / 2, OrthoWidth / 2, -OrthoHeight / 2,
                      OrthoHeight / 2, NearClip, FarClip);
}
...
void CameraComponent::LateUpdate(float dt) {
  if (Target) Owner->transform.Position = Target->transform.Position + Offset;
}
