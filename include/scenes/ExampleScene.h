...
#include "engine/components/ProtonParticle.h"
#include "engine/components/ElectronParticle.h"
#include "engine/components/NeutronParticle.h"
...
public:
    ...
    void Start() override
    {
        ...
        chargeGrid.resize(gridResolution, std::vector<std::vector<float>>(gridResolution, std::vector<float>(gridResolution, 0.0f)));
    ...
    void SpawnParticles()
    {
        ...
        for (int i = 0; i < protonCount; i++)
        {
            ...
            auto mesh = obj->AddComponent<MeshComponent>(std::make_unique<SphereMesh>(6, 6));
    ...
    glm::ivec3 WorldToGrid(const glm::vec3 &pos)
    {
        ...
        int x = glm::clamp(int((pos.x + halfSize) / gridSize * gridResolution), 0, gridResolution - 1);
        int y = glm::clamp(int((pos.y + halfSize) / gridSize * gridResolution), 0, gridResolution - 1);
        int z = glm::clamp(int((pos.z + halfSize) / gridSize * gridResolution), 0, gridResolution - 1);
            ...
            for (int dx = -neighborRange; dx <= neighborRange; dx++)
                for (int dy = -neighborRange; dy <= neighborRange; dy++)
                    for (int dz = -neighborRange; dz <= neighborRange; dz++)
                    {
                        ...
                        force += glm::normalize(delta) * (k * p->charge * q) / (dist * dist);
                    }

            // clamp and apply
            float fMag = glm::length(force);
            if (fMag > maxForce)
                force = force / fMag * maxForce;
            p->AddForce(force);
        }
    }
    ...
    {
        if (dynamic_cast<ElectronParticle *>(a) || dynamic_cast<ElectronParticle *>(b))
            return;

        glm::vec3 delta = b->Owner->Transform.Position - a->Owner->Transform.Position;
        float dist = glm::length(delta);
        ...
        float forceMag = 1.0f / glm::pow(dist, 12) - attraction / dist * glm::exp(-dist / wellDist);
    ...
    void AddTemp(float tempStrength)
    {
        for (auto *p : particles)
        {
            if (dynamic_cast<ProtonParticle *>(p) || dynamic_cast<NeutronParticle *>(p))
            {
